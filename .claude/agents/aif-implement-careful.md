...
description: The implementation station of the aif foundry, careful tier. Byte-for-byte the same instructions as aif-implement, on the careful engine. Dispatched by `aif work` when the ticket's risk is high — where a passing suite is not on its own sufficient evidence of correctness. Not for direct use.
