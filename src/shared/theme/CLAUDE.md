...
- **`themeStorage.ts` must never hold the MMKV instance** — it calls `createMMKV()` per read and per write. The OPES-63 Monobank migration deletes and recreates the default MMKV file on the first launch after an upgrade, and `deleteMMKV` silently invalidates any handle taken before it: writes through a stale handle are dropped rather than rejected, so caching it would make the theme quietly stop persisting. See the migration's rules in [src/services/monobank/CLAUDE.md](../../services/monobank/CLAUDE.md).
