...
- **Deleting the key is only half of it — the store is then rebuilt.** MMKV is an append-only log in an mmap: `remove` appends a zero-length record that shadows the earlier one and never overwrites the original bytes. Verified on device: after a migration that satisfied every other criterion, `strings Documents/mmkv/mmkv.default` still printed the token. Neither `trim()` nor `clearAll()` erases it. So once a run has deleted at least one key, `purgeDeletedRecords()` snapshots the surviving keys, calls `deleteMMKV('mmkv.default')` and writes them back into the fresh file. It runs **once, after the last delete** — the rebuild drops the backing file, so doing it between the two keys would invalidate the handle the second delete needs. It is skipped entirely when the run deleted nothing, and a throw from it is caught: leaving the residue is no worse than not having run, and must not stop the app launching.
- **The rebuild fails closed.** It restores values through `getString`, so a key holding a non-string would be dropped by the rebuild. If any surviving key cannot be read back as a string it abandons the purge and leaves the residue rather than lose the value.
- **Nothing may cache a handle to the default MMKV instance.** `deleteMMKV` silently invalidates every handle taken before it — writes through a stale one are dropped, not rejected. [themeStorage.ts](../../shared/theme/themeStorage.ts) and [MonobankAccountSelectionService.ts](MonobankAccountSelectionService.ts) therefore call `createMMKV()` per operation instead of holding the instance at module scope. MMKV caches live instances by id, so that is a lookup rather than a reopen. Keep it that way for any new plaintext-backed storage.
