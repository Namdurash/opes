...
This repo runs a spec-driven AI SDLC: **non-trivial work is documented and driven through OpenSpec**, then committed straight to `main` per phase. Tooling is the `openspec` CLI plus the `/opsx:*` commands and `openspec-*` skills. **Do not edit those vendored files** — they are regenerated by `openspec update`. Project conventions live in [openspec/config.yaml](openspec/config.yaml) (machine-readable, fed into every artifact) and in this file.
...
**Trunk-based — never create a branch.** Every commit lands directly on `main`.
...
- Committed to `main` per the **Commit convention** (no branches).
