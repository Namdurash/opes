# subscription-detective Specification

## Purpose
Define the developer-only Subscription Detective agent harness: a self-contained loop (driven by `@anthropic-ai/sdk`) that detects recurring subscriptions in a bundled mock transaction dataset. It is run via `npm run agent:dev`, never reads the app's real data, and conforms to the project's TypeScript/style and layer conventions. Wiring the agent to real data and the in-app feature are out of scope here.
