...
## Purpose
Define how a user views and manages payment cards beyond creation and reordering. A user can open any card to see its details; manual cards (created in-app, no Monobank account id) can be edited or deleted, while Monobank-synced cards are owned by the sync source and stay read-only. Tapping a card must coexist with the existing long-press drag-to-reorder gesture.
