...
## Purpose
Define the build-health invariant the project maintains: the codebase always satisfies the Definition of Done (`tsc --noEmit`, `lint`, and `npm test` pass on `main`), so the AI SDLC's per-phase commit gate never blocks on a broken build.
