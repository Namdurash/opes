...
{ "schema": 1,
  ...
  "decisions": [
    ...
    { "id": "D-017",
      "statement": "Add exactly one new member to useSettingsStore, `resetSandbox: () => Promise<void>`; the action awaits resetSandboxEnvironment(), then calls useMonobankStore.getState().disconnect().",
      "because": "disconnect() returns the monobank and transactions stores to defaults, which is AS-016",
      "rejected": "Do not add an isResettingSandbox flag beside isExporting; no criterion asserts a busy state and AS-013 makes a second run the recovery path." },
    { "id": "D-018",
    ...
    { "id": "D-032",
      "statement": "Give the reset Pressable exactly three props — testID, style and onPress; no `disabled`, no loading affordance, no text that changes while the action runs.",
      "because": "the store exposes no busy flag, so the row has nothing to read" },
    { "id": "D-033",
      "statement": "Let onPress call the action unguarded; add no in-flight check and no early return, and let a second press run the reset a second time.",
      "because": "AS-013 — a second run reaches the same end state, which AC-012 asserts" } ],
  "ac_coverage": {
